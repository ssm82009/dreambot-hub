...
const DreamForm = () => {
  ...
  useEffect(() => {
    if (userSubscription && pricingSettings && subscriptionUsage) {
      ...
      setHasReachedLimit(totalAllowed !== -1 && subscriptionUsage.interpretations_used >= totalAllowed);
    }
  ...
  const renderInterpretationLimitStatus = () => {
    ...
    const totalAllowed = getTotalInterpretations(userSubscription, pricingSettings);
    if (totalAllowed === -1) return null; // Unlimited interpretations
